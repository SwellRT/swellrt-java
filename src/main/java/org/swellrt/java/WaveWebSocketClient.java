...
public class WaveWebSocketClient implements WaveSocket.WaveSocketCallback {
  ...
  final static Logger Log = LoggerFactory.getLogger(WaveWebSocketClient.class);
  ...
  public void connect(ConnectionListener listener) {
    ...
    if (connected == ConnectState.DISCONNECTED) {
      Log.debug("Attemping to reconnect");
    ...
    if ("ProtocolWaveletUpdate".equals(messageType)) {
      if (callback != null) {
        ...
        try {
          ...
          Log.debug("Error parsing WaveletUpdate JSON message", e);
          return;
        }
      ...
      if (callback != null) {
        ...
        try {
          ...
          Log.error("Error parsing SubmitResponse JSON message", e);
          return;
        }
